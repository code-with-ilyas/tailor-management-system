...
<div class="container d-flex justify-content-center align-items-center my-5" style="min-height: 80vh;">
    <div class="card shadow-sm w-100" style="max-width: 600px;">
        ...
        <h5 class="mb-0">
    <a href="{{ route('pents.index') }}" class="text-decoration-none" style="font-size: 14px; color: blue;">Show Pents Measurements</a>
</h5>
            ...
            <form id="pentsForm" action="{{ route('pents.store') }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control" id="pent_length" name="pent_length" value="{{ old('pent_length') }}" required>
                        @error('pent_length') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-6">
                        <label for="waist" class="form-label">Waist</label>
                        <input type="number" step="0.01" class="form-control" id="waist" name="waist" value="{{ old('waist') }}" required>
                        @error('waist') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-6">
                        <label for="hips" class="form-label">Hips</label>
                        <input type="number" step="0.01" class="form-control" id="hips" name="hips" value="{{ old('hips') }}" required>
                        @error('hips') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-6">
                        <label for="inseam" class="form-label">Inseam</label>
                        <input type="number" step="0.01" class="form-control" id="inseam" name="inseam" value="{{ old('inseam') }}" required>
                        @error('inseam') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-6">
                        <label for="pensa" class="form-label">Pensa</label>
                        <input type="number" step="0.01" class="form-control" id="pensa" name="pensa" value="{{ old('pensa') }}" required>
